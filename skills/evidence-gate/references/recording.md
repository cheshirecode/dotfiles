...
[health-evidence.md](health-evidence.md).
...
**Capture before parsing: exit 1 is the verdict, not a failure.** Run
`out=$(python3 <skill-dir>/scripts/evidence_gate.py check --gate <gate-file>); rc=$?`
and branch on `$rc`. Never pipe `check` (`check | tail`) under `set -o pipefail`,
and never chain `check && claim-complete` — both silently reinterpret the
uncovered verdict as a broken command.
