...
To dogfood a cost comment on an open PR, follow "Comment cost on an open PR"
in [SKILL.md](SKILL.md). That recipe sums `message.usage` from the session
JSONL and runs `annotate` with `PR_COST_HOOK_LIVE=1`.
