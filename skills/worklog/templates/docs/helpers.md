...
- `boundary-lint.sh` — **read-only**, scans task/project markdown for clone-boundary drift declared in `.worklog-boundary.json`. This is for split-source hygiene (for example: OSS tracker terms appearing in a work tracker, or work task vocabulary appearing in an OSS tracker), not schema correctness. Profile shape: `schema`, `label`, `include`/`exclude` globs, `deny: [{pattern,note}]`, optional `allow: [{path,pattern}]`, and `ignore_case` (default true). Exit 1 on matches. `bin/audit.sh --section=boundary` runs it automatically when a profile exists.
