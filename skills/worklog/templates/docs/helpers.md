...
- `audit.sh` — **read-only** composite health report. Sections: stale (`stale.sh`), blocked ≥7d, in-review ≥14d, cross-task drift (`lint --cross-task`). Always exits 0 — report, not gate. Use `--section=<name>` to scope.
