...
```bash
gh repo clone <work-org>/_worklog ~/Documents/projects/_worklog
cd ~/Documents/projects/_worklog && bin/install-hooks.sh --write
```
...
- **Branching:** none. Commit to `main` from every machine. Linear history during normal sync — `git pull --no-rebase`, never rebase. Maintenance ops (log compaction, cache purge) *do* rewrite history; they tag `pre-<op>-<ts>` and `bin/post-rewrite-prompt.sh` emits the recovery prompt for other clones. See `AGENTS.md` § Editing rules.
- **Helpers:** `bin/checkpoint.sh` (per-task update), `bin/archive.sh` (ship/supersede), `bin/autosave.sh` (slugless snapshot), `bin/status.sh` (standup summary), `bin/context.sh` (per-task context pack), `bin/init-scan.sh` (read-only exact-scan seeds for `/worklog init --full`), `bin/lint.sh` (per-file format + `--cross-task` for stale-review / blocked-FSM / undeclared-body-ref drift).
- **Vault shape:** `_worklog/` opens as an Obsidian vault — folder of `.md` files with YAML frontmatter; no `.obsidian/` is checked in, so a fresh open creates a default workspace. The primary value is **Dataview queries over frontmatter** (consistent `slug` / `status` / `kind` / `last_updated` / `project` etc.). Caveat: relational edges (`parent_slug`, `related`, `supersedes`, `superseded_by`) are stored as **bare slugs** because grep-as-index is load-bearing (see `AGENTS.md` "Slug as join key"). Obsidian's core graph won't traverse bare-slug frontmatter values — to get graph edges from these, install Dataview (link-coercion via `dv.pages()`) or a plugin like *Frontmatter Links*. Backlinks are sparse today since body text rarely uses `[[wikilinks]]` (the `auto-slug-link` idea in `archive/worklog-review-brainstorm.md` is the future path). Slug-grep + `git log --grep="Worklog-Slug: <slug>"` remains the authoritative join.

## Claude Code users

One skill, ten modes — see `~/.claude/skills/worklog/SKILL.md`. Bare `/worklog` (or `/worklog help`) prints the subcommand menu.
...
See [`docs/codex-setup.md`](./docs/codex-setup.md) for install + the manual session-end flow (Codex has no hooks equivalent; `bin/autosave.sh` + `bin/compact-kernels.sh` are run by hand before ending a conversation).
...
- If it does not exist yet, read this repo's `AGENTS.md`, use the kickoff
  prompt below, and route `worklog ...` to the `_worklog/bin/*` helpers
...
helpers in `bin/` plus the protocol in `AGENTS.md`. Do not answer with a
...
- `worklog init` → repo setup + LDAP resolution + `bin/init-scan.sh`
- `worklog sync <slug>` → usually `bin/checkpoint.sh <slug> ...` or
  `bin/archive.sh <slug> ...`; if no slug is discoverable, `bin/autosave.sh`
- `worklog status ...` → `bin/status.sh ...`, then synthesize the result
  into prose per `AGENTS.md`
- `worklog context <slug> ...` → `bin/context.sh <slug> ...`
...
```
Set up <work-org>/_worklog as a sibling of the current repo. If the clone is
...
- run `bin/init-scan.sh --format=json`
...
Helpers live in `bin/` — each script self-documents via `--help`:
  checkpoint.sh  archive.sh  autosave.sh  status.sh  context.sh  init-scan.sh
```
