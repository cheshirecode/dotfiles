...
export function renderHtml(graph) {
  ...
  const statesInGraph = new Set(graph.nodes.map((node) => node.state));
  const relationsInGraph = new Set(graph.edges.map((edge) => edge.relation));
  const defaultRelations = new Set(["parent", "project", "reopens", "supersedes", "superseded_by", "depends_on"]);
  const checkedState = (state) => (state === "active" || state === "project" || (filterActive && statesInGraph.has(state))) ? " checked" : "";
  const checkedRelation = (relation) => (defaultRelations.has(relation) || (filterActive && relationsInGraph.has(relation))) ? " checked" : "";
  return `<!doctype html>
<html lang="en">
<head>
<meta charset="utf-8">
...
<style>
html,body{width:100%;height:100%;overflow:hidden}*{box-sizing:border-box}body{margin:0;background:#f5f7fa;color:#172033;font-family:ui-sans-serif,system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif}
...
h1{font-size:24px;margin:0}.meta{font-size:12px;color:#5d6778;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.controls{display:flex;gap:8px;flex-wrap:wrap;align-items:center;background:#fff;border-bottom:1px solid #d7dce4;padding:8px 12px;min-width:0}.controls label,.chip{font-size:13px;white-space:nowrap}.controls label{display:inline-flex;align-items:center;gap:5px;min-height:32px;padding:2px 0}.controls input[type=checkbox]{width:18px;height:18px;flex:0 0 auto}.controls input[type=search]{width:min(320px,100%);padding:7px 9px;border:1px solid #bec7d4;border-radius:6px}.controls button{border:1px solid #bec7d4;background:#fff;border-radius:6px;padding:7px 9px;color:#172033;cursor:pointer}.controls button:hover{border-color:#64748b}.count{color:#64748b}
...
#stage{position:relative;overflow:hidden;background:#eef1f5;min-width:0;min-height:0;touch-action:none;cursor:grab}#stage.panning{cursor:grabbing}#canvas{position:absolute;left:0;top:0;transform-origin:0 0;will-change:transform}#edges{position:absolute;inset:0;pointer-events:none;overflow:visible}.edgeLabel{font-size:11px;font-weight:700;fill:#334155;paint-order:stroke;stroke:#eef1f5;stroke-width:4px;stroke-linejoin:round;pointer-events:none}
.node{position:absolute;width:230px;min-height:78px;box-sizing:border-box;border:1px solid #c6ceda;border-radius:8px;background:#fff;box-shadow:0 2px 8px rgba(24,34,52,.08);padding:11px 12px;cursor:pointer}.node.parentNode{background:#f8fbff;border-color:#9fb0c6;box-shadow:0 4px 14px rgba(24,34,52,.13)}.node.childNode{background:#fff}.node:hover,.node.selected{border-color:#334155;box-shadow:0 5px 16px rgba(24,34,52,.16)}.node.selected{outline:2px solid #172033;outline-offset:2px}.slug{font-weight:700;font-size:14px;overflow-wrap:anywhere}.tags{margin-top:7px;display:flex;gap:5px;flex-wrap:wrap}.tag{font-size:11px;color:#334155;background:#eef2f7;border-radius:999px;padding:2px 6px}.empty{position:absolute;inset:0;display:grid;place-items:center;color:#5d6778;text-align:center;padding:24px}
aside{border-left:1px solid #d7dce4;background:#fff;overflow:auto;min-width:0}#details{padding:14px;border-bottom:1px solid #d7dce4;font-size:13px}#details h2{font-size:20px;margin:0 0 8px;overflow-wrap:anywhere}.field{margin-top:4px}.edgeGroup{margin-top:10px}.edgeGroup strong{display:block;margin-bottom:3px}.edgeGroup div{color:#334155;line-height:1.35}.reveal{margin:4px 6px 0 0;border:1px solid #bec7d4;background:#f8fafc;border-radius:6px;padding:4px 7px;color:#172033;cursor:pointer}.diagSummary{border-bottom:1px solid #d7dce4;background:#fff7ed;color:#7c2d12;font-size:12px}.diagSummary summary{cursor:pointer;padding:10px 14px;font-weight:700}.diag{padding:0 14px 10px}.diag+.diag{border-top:1px solid #fed7aa;padding-top:10px}.diag strong{display:block;margin-bottom:4px}#debug{border-top:1px solid #d7dce4}#debug summary{padding:10px 14px;cursor:pointer;color:#334155;font-size:12px}pre{margin:0;padding:14px;overflow:auto;font-size:11px;line-height:1.45;background:#fbfcfe;max-height:34vh}
@media(max-width:900px){body{grid-template-rows:auto auto minmax(0,1fr)}header{display:block;padding:10px 12px}.meta{white-space:normal;overflow-wrap:anywhere}.controls{display:flex;flex-wrap:wrap;gap:6px;overflow:visible;padding:8px 10px}.controls input[type=search]{flex:1 1 180px;width:auto;min-width:0}.controls button{flex:0 0 auto;width:auto;padding:6px 8px}.controls label{display:inline-flex;align-items:center;gap:4px;flex:0 1 auto;min-width:0;min-height:28px;font-size:12px;white-space:nowrap}.controls input[type=checkbox]{width:16px;height:16px}.chip{flex:1 0 100%;white-space:nowrap}.count{flex:0 0 auto}main{grid-template-columns:1fr;grid-template-rows:minmax(0,1fr)minmax(180px,40dvh)}body.details-collapsed main{grid-template-rows:minmax(0,1fr)0}aside{border-left:0;border-top:1px solid #d7dce4}.node{width:216px}}
</style>
</head>
<body>
...
<label><input type="checkbox" data-state="active"${checkedState("active")}> active <span class="count" id="count-state-active"></span></label>
<label><input type="checkbox" data-state="archive"${checkedState("archive")}> archive <span class="count" id="count-state-archive"></span></label>
<label><input type="checkbox" data-state="project"${checkedState("project")}> projects <span class="count" id="count-state-project"></span></label>
<label><input type="checkbox" data-relation="parent"${checkedRelation("parent")}> parent <span class="count" id="count-rel-parent"></span></label>
<label><input type="checkbox" data-relation="related"${checkedRelation("related")}> related <span class="count" id="count-rel-related"></span></label>
<label><input type="checkbox" data-relation="project"${checkedRelation("project")}> project <span class="count" id="count-rel-project"></span></label>
<label><input type="checkbox" data-relation="reopens"${checkedRelation("reopens")}> reopens <span class="count" id="count-rel-reopens"></span></label>
<label><input type="checkbox" data-relation="supersedes"${checkedRelation("supersedes")}> supersedes <span class="count" id="count-rel-supersedes"></span></label>
<label><input type="checkbox" data-relation="superseded_by"${checkedRelation("superseded_by")}> superseded <span class="count" id="count-rel-superseded_by"></span></label>
<label><input type="checkbox" data-relation="depends_on"${checkedRelation("depends_on")}> depends on <span class="count" id="count-rel-depends_on"></span></label>
...
function diagnosticsHtml(){const diagnostics=graph.diagnostics||[];if(!diagnostics.length)return"";const grouped=new Map();for(const d of diagnostics){const key=[d.level,d.code,d.message,d.file].join("\\n");const item=grouped.get(key)||{...d,count:0};item.count+=1;grouped.set(key,item)}const items=[...grouped.values()];const hasError=items.some(d=>d.level==="error");return'<details class="diagSummary" '+(hasError?'open':'')+'><summary>'+diagnostics.length+' diagnostics, '+items.length+' grouped</summary>'+items.slice(0,8).map(d=>'<div class="diag"><strong>'+esc(d.level+' '+d.code)+(d.count>1?' x'+d.count:'')+'</strong>'+esc(d.message)+'<br>'+esc(d.file||'')+'</div>').join("")+(items.length>8?'<div class="diag">... '+(items.length-8)+' more groups</div>':'')+'</details>'}
document.getElementById("diagnostics").innerHTML=diagnosticsHtml();
function hay(n){return [n.slug,n.status,n.kind,n.okfType,n.worklogId,n.timestamp,n.project,n.file,...(n.repos||[])].join(" ").toLowerCase()}
function countBy(items,keyFn){const out={};for(const item of items){const k=keyFn(item)||"";out[k]=(out[k]||0)+1}return out}
function nodeTags(n){const raw=n.type==="project"?[n.type]:[n.status||n.state,n.kind,n.ldap];const out=[];for(const item of raw){const value=String(item||"").trim();if(value&&!out.includes(value))out.push(value)}return out.length?out:["-"]}
...
function relationColor(rel){return({parent:"#2563eb",project:"#0f766e",related:"#64748b",reopens:"#d97706",supersedes:"#7c3aed",superseded_by:"#7c3aed",depends_on:"#0891b2"}[rel]||"#64748b")}
function markerId(rel){return"arrow-"+String(rel).replace(/[^a-zA-Z0-9_-]/g,"_")}
function addEdgeMarkers(edges){const rels=[...new Set(edges.map(e=>e.relation))],defs=document.createElementNS("http://www.w3.org/2000/svg","defs");for(const rel of rels){const marker=document.createElementNS("http://www.w3.org/2000/svg","marker"),head=document.createElementNS("http://www.w3.org/2000/svg","path");marker.setAttribute("id",markerId(rel));marker.setAttribute("viewBox","0 0 10 10");marker.setAttribute("refX","9");marker.setAttribute("refY","5");marker.setAttribute("markerWidth","7");marker.setAttribute("markerHeight","7");marker.setAttribute("orient","auto");head.setAttribute("d","M 0 0 L 10 5 L 0 10 z");head.setAttribute("fill",relationColor(rel));marker.appendChild(head);defs.appendChild(marker)}edgeSvg.appendChild(defs)}
...
function render(){const vg=visible(),pos=layout(vg.nodes,vg.edges);lastVisible=vg;canvas.querySelectorAll(".node,.empty").forEach(n=>n.remove());edgeSvg.innerHTML="";addEdgeMarkers(vg.edges);let maxX=stage.clientWidth,maxY=stage.clientHeight;for(const p of pos.values()){maxX=Math.max(maxX,p.x+CARD_W+PAD);maxY=Math.max(maxY,p.y+CARD_H+PAD)}canvasSize={w:maxX,h:maxY};canvas.style.width=maxX+"px";canvas.style.height=maxY+"px";edgeSvg.setAttribute("width",maxX);edgeSvg.setAttribute("height",maxY);if(!vg.nodes.length){const empty=document.createElement("div");empty.className="empty";empty.textContent="No visible tasks. Relax filters or search another slug.";canvas.appendChild(empty)}const parentIds=new Set(vg.edges.filter(e=>e.relation==="parent").map(e=>e.source)),childIds=new Set(vg.edges.filter(e=>e.relation==="parent").map(e=>e.target));for(const n of vg.nodes){const p=pos.get(n.id),color=statusColors[n.status]||statusColors[n.state]||"#64748b";const el=document.createElement("div");el.className="node"+(parentIds.has(n.id)?" parentNode":"")+(childIds.has(n.id)?" childNode":"")+(n.id===selectedId?" selected":"");el.dataset.id=n.id;el.style.left=p.x+"px";el.style.top=p.y+"px";el.style.borderColor=color;el.innerHTML='<div class="slug">'+esc(n.slug)+'</div><div class="tags">'+nodeTags(n).map(tag=>'<span class="tag">'+esc(tag)+'</span>').join("")+'</div>';el.addEventListener("click",ev=>{ev.stopPropagation();selectedId=n.id;search.value=n.slug;if(window.matchMedia("(max-width:900px)").matches){document.body.classList.remove("details-collapsed");updateDetailsToggle()}render();selectNode(n,lastVisible)});canvas.appendChild(el)}for(const e of vg.edges){const a=pos.get(e.source),b=pos.get(e.target);if(!a||!b)continue;let x1=a.x+CARD_W/2,y1=a.y+CARD_H/2,x2=b.x+CARD_W/2,y2=b.y+CARD_H/2;const dx=x2-x1,dy=y2-y1;if(Math.abs(dx)>Math.abs(dy)){const sx=dx>=0?1:-1;x1+=sx*CARD_W/2;x2-=sx*CARD_W/2}else{const sy=dy>=0?1:-1;y1+=sy*CARD_H/2;y2-=sy*CARD_H/2}const path=document.createElementNS("http://www.w3.org/2000/svg","path");path.setAttribute("d","M "+x1+" "+y1+" C "+x1+" "+((y1+y2)/2)+", "+x2+" "+((y1+y2)/2)+", "+x2+" "+y2);path.setAttribute("stroke",relationColor(e.relation));path.setAttribute("stroke-width",e.relation==="parent"?"2":(selectedId&&(e.source===selectedId||e.target===selectedId)?"2.2":"1.2"));path.setAttribute("fill","none");path.setAttribute("opacity",e.relation==="parent"?".72":(selectedId&&(e.source!==selectedId&&e.target!==selectedId)?".22":".5"));path.setAttribute("marker-end","url(#"+markerId(e.relation)+")");edgeSvg.appendChild(path);const label=document.createElementNS("http://www.w3.org/2000/svg","text");label.classList.add("edgeLabel");label.setAttribute("x",(x1+x2)/2);label.setAttribute("y",(y1+y2)/2-6);label.setAttribute("text-anchor","middle");label.textContent=e.relation.replaceAll("_"," ");edgeSvg.appendChild(label)}document.getElementById("visibleStats").textContent=vg.nodes.length+" visible nodes, "+vg.edges.length+" visible edges";if(!userMoved)fitView();if(selectedId){const node=graph.nodes.find(n=>n.id===selectedId);if(node)selectNode(node,vg)}else resetDetails()}
function groupName(e,n){if(e.relation==="parent")return e.target===n.id?"Parents":"Children";if(e.relation==="depends_on")return e.target===n.id?"Depends on":"Required by";if(e.relation==="project")return "Same project";if(e.relation==="related")return "Related";if(e.relation==="reopens")return "Reopens";if(e.relation==="supersedes"||e.relation==="superseded_by")return "Supersedes";return e.relation}
...
const detailsToggle=document.getElementById("toggleDetails");function updateDetailsToggle(){detailsToggle.textContent=document.body.classList.contains("details-collapsed")?"Show details":"Hide details"}if(window.matchMedia("(max-width:900px)").matches)document.body.classList.add("details-collapsed");updateDetailsToggle();
document.getElementById("fit").addEventListener("click",fitView);document.getElementById("reset").addEventListener("click",resetView);document.getElementById("zoomIn").addEventListener("click",()=>zoomAt(1.18,stage.clientWidth/2,stage.clientHeight/2));document.getElementById("zoomOut").addEventListener("click",()=>zoomAt(.85,stage.clientWidth/2,stage.clientHeight/2));detailsToggle.addEventListener("click",()=>{document.body.classList.toggle("details-collapsed");updateDetailsToggle()});
