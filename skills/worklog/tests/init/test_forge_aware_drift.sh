...
mk_clone "$TMP/clones/example-repo" "git@gitlab.com:examplens/example-repo.git"
...
if [[ "$1" == mr && "$2" == list ]]; then
  # Only the MR list for examplens/example-repo is populated.
  for a in "$@"; do [[ "$a" == "examplens/example-repo" ]] && found=1; done
  if [[ -n "${found:-}" ]]; then
    echo '[{"iid":1770,"title":"splus-19029 payout guard","web_url":"https://gitlab.com/examplens/example-repo/-/merge_requests/1770"}]'
  else
    echo '[]'
  fi
  exit 0
...
out="$("$FORGE" list --author fred.tran "$TMP/clones/example-repo" "$TMP/clones/dotfiles" 2>/dev/null)"
...
printf '%s' "$out" | grep -Eq $'^open\tgitlab\texamplens/example-repo\t1770\t'
...
st="$("$FORGE" state "$TMP/clones/example-repo" 1770 2>/dev/null)"
...
export PATH="$MIRROR"
out2="$("$FORGE" list --author fred.tran "$TMP/clones/example-repo" "$TMP/clones/dotfiles" 2>/dev/null)"
if [[ -n "$out2" ]]; then rc=0; else rc=1; fi
check "no-CLI case still produces output instead of an empty drift block" "$rc"
printf '%s' "$out2" | grep -q $'^gap\tgitlab\texamplens/example-repo\tglab-not-installed$'
...
# /workspace/example-repo and /workspace/example-repo-wt-mockfix are both clones of
# examplens/example-repo, so a per-clone loop queries the project twice and every MR is
# reported twice. A reader cross-referencing 10 rows against 5 tracked MRs sees
# drift that is not there.
export PATH="$TMP/stub:$MIRROR"
git init -q "$TMP/clones/example-repo-wt" 2>/dev/null
git -C "$TMP/clones/example-repo-wt" remote add origin "https://gitlab.com/examplens/example-repo.git"
dup="$("$FORGE" list --author fred.tran "$TMP/clones/example-repo" "$TMP/clones/example-repo-wt" 2>/dev/null \
       | grep -c $'^open\tgitlab\texamplens/example-repo\t1770\t' || true)"
[[ "$dup" == 1 ]]
