...
grep -q 'Active Next Actions' "$HTML_OUT"
grep -q 'id="nextActions"' "$HTML_OUT"
grep -q 'function renderNextActions()' "$HTML_OUT"
grep -q 'data-next-action-row' "$HTML_OUT"
grep -q 'Finish projects child.' "$HTML_OUT"
grep -q 'Render projects child.' "$HTML_OUT"
