...
- There is no `project.sh add-child`. Extra work mid-run (a rereview after HEAD moved, a later wave) is a pre-declared `*-r2` child at `project new`, or a new wave project. Do not create a standalone task with `project:` set but missing from the parent's `tasks:` block — `verify` will drift and `next` will not see it.
