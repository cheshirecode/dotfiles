...
- **`WORKLOG_CHECKPOINT_FORCE=1` is the one-shot bypass for the exit-1 guard, and the last resort.** Use it only for a deliberately multi-purpose commit, as an explicit, stated-reason override — reach for separate commits, `--include=<path>`, or `git restore --staged <path>` first, in that order (`checkpoint.sh`'s own refusal message lists them that way). Callers must not present the forced form as their default. This skill owns these semantics; other skills route here rather than restating them.
