...
- Theme: resolve theme reactively (`useThemeMode`/`MiniAppThemeProvider`), never snapshot once. 3P reads `documentElement[data-theme]`; 1P reads `localStorage["<WORK_APP>_THEME_MODE"]` + `matchMedia`. Map app CSS vars → Nova tokens scoped to the mount root; never hardcode `font`/`color`/`background` on `body` (a 1P MFE leaks it into the host doc).
